...
Wenn nun bei gleicher Vola die Rendite niedriger ist, kann der Grenzwert auch negativ werden. Hier zum Beispiel mit Erwartungswert 1% für die Performance im Einzelschritt. Die langfristige Entwicklung ist dann negativ. Betrachtet man die Entwicklung eines vorgegebenen Anfangsvermögen, so sinkt dieses exponentiell.
...
patchwork::wrap_plots(
  ggplot(Finanzdaten_niedrig) +
    ...
    coord_cartesian(xlim = c(0, anzeige_bis_schritt)),
  ggplot(Finanzdaten_niedrig) +
    geom_line(
...
Nun kombinieren wir mehreren solchen Anlagen zu einem Portfolio, d.h. wir kaufen zu Beginn gleiche Anteile von allen verfügbaren Anlagen. Diese können wir dann entweder einfach halten ('plain portfolio') oder wir gleichen nach jedem Zeitschritt die Anteile wieder an so dass der Anteil der einzelnen Anlagen am Portfolio immer gleich ist ('balanced portfolio'). 

```{r}
#| label: verlauf-portfolien
#| fig-width: 7
#| fig-height: 4
...
patchwork::wrap_plots(
  ggplot(Finanzdaten_niedrig, aes(x = Schritt)) +
    geom_line(
      aes(
        y = GeoMittel - 1, color = factor(Wiederholung), linewidth = "asset"
      ),
      key_glyph = "timeseries"
    ) +
    geom_line(
      data = Portfolio,
      mapping = aes(
        y = balanced_GeoMittel - 1, color = "balanced portfolio", linewidth = "portfolio"
      ),
      key_glyph = "timeseries"
    ) +
    geom_line(
      data = Portfolio,
      mapping = aes(
        y = sum_GeoMittel - 1, color = "plain portfolio", linewidth = "portfolio"
      ),
      key_glyph = "timeseries"
    ) +
    labs(
    ...
    scale_linewidth_manual(
      values = c("asset" = .1, "portfolio" = 1),
      guide = "none"
    ) +
    guides(color = guide_legend(override.aes = list(linewidth = c(rep(.5, wdh), 1, 1)))) +
    theme_minimal() +
    theme(
      axis.title.x = element_text(hjust = 0.015),
      # axis.title.y = element_text(hjust = 0.08),
    ...
    coord_cartesian(
      xlim = c(0, anzeige_bis_schritt),
      ylim = c(-.1, .1)
    ),
  ggplot(Finanzdaten_niedrig, aes(x = Schritt)) +
    geom_line(
      aes(
        y = X_prod, color = factor(Wiederholung), linewidth = "asset"
      ),
      key_glyph = "timeseries"
    ) +
    geom_line(
      data = Portfolio,
      mapping = aes(
        y = balanced_prod, color = "balanced portfolio", linewidth = "portfolio"
      ),
      key_glyph = "timeseries"
    ) +
    geom_line(
      data = Portfolio,
      mapping = aes(
        y = sum_prod, color = "plain portfolio", linewidth = "portfolio"
      ),
      key_glyph = "timeseries"
    ) +
    labs(
...
Wenn wir also nach jedem Schritt etwas von denjenigen Anlagen verkaufen, die 'gut gelaufen' sind und dafür von denen die 'schlecht gelaufen' sind nachkaufen, können wir die Rendite des Portfolios deutlich erhöhen!
